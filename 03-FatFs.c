...
static void run_cam_defaults(const size_t argc, const char *argv[]){
    if (!expect_argc(argc, argv, 0)) return;
    for (uint16_t i = 0; sensor_default_regs[i][0] != 0x0000; i++)
	{
		OV5640_WR_Reg(i2c1,0x3C,sensor_default_regs[i][0],sensor_default_regs[i][1]);
	} 
}
...
static void run_cam_capture(const size_t argc, const char *argv[]) {
    ...
    // open file on SD
    FIL fil;
    FRESULT fr = f_open(&fil, argv[0], FA_WRITE | FA_CREATE_ALWAYS);
    if (FR_OK != fr) {
        printf("f_open error: %s (%d)\n", FRESULT_str(fr), fr);
        return;
    }

    // write the raw frame buffer
    UINT bw;
...
static cmd_def_t cmds[] = {
    ...
    {"cam_xclk", run_cam_xclk,
     "cam_xclk [<freq_khz>]:\n"
     " Set camera XCLK frequency. Default 37000\n"
     "\te.g.: cam_xclk 37000"},
    ...
    {"cam_defaults", run_cam_defaults,
     "cam_defaults:\n"
     " Write default register set to camera"},
