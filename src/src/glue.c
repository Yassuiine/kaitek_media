...
// Keep the SD/FatFs glue quiet during normal operation. The per-call traces are
// useful when bringing up storage, but they overwhelm the interactive shell.
...
DRESULT disk_read(BYTE pdrv,  /* Physical drive number to identify the drive */
    ...
    if (SD_BLOCK_DEVICE_ERROR_NONE != rc) {
        printf("disk_read error: rc=%d pdrv=%u sector=%llu count=%u\n",
               rc, pdrv, (unsigned long long)sector, count);
    }
...
DRESULT disk_write(BYTE pdrv, /* Physical drive number to identify the drive */
    ...
    if (SD_BLOCK_DEVICE_ERROR_NONE != rc) {
        printf("disk_write error: rc=%d pdrv=%u sector=%llu count=%u\n",
               rc, pdrv, (unsigned long long)sector, count);
    }
    return sdrc2dresult(rc);
}
...
DRESULT disk_ioctl(BYTE pdrv, /* Physical drive number (0..) */
    ...
    switch (cmd) {
        ...
        case CTRL_SYNC:
            ...
            return RES_OK;
        default:
            printf("disk_ioctl unsupported cmd=%u pdrv=%u\n", cmd, pdrv);
            return RES_PARERR;
    }
}
