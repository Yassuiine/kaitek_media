...
static block_dev_err_t sd_cmd(sd_card_t *sd_card_p, const cmdSupported cmd, uint32_t arg,
    ...
    if ((uint)-1 != sd_card_p->spi_if_p->ss_gpio &&
        0 != gpio_get(sd_card_p->spi_if_p->ss_gpio)) {
        // Be tolerant if a previous transaction released CS before this command.
        // The SPI SD protocol requires CS low for command framing, so recover
        // by selecting the card instead of hard-asserting here.
